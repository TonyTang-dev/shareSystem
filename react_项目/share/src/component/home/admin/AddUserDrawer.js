...
const AddUser = (props) => {
    const {visible, setVisible, beRefresh, setRefreshList} = props;
    ...
    const onSubmitBtn=(event)=>{
        ...
        $.ajax({
            url:"/shareArticle/user/adduseradmin.map",
            type:"POST",
            cache: false,
            dataType: "json",
            data: obj,
    ...
    // 选择拓展菜单
    const selectExtendMenu=(e)=>{
        let tempdata = [];
        for(let i of customMenu){
            if(i === e){
                return;
            }
            tempdata.push(i);
        ...
        <>
            ...
            <Drawer
                ...
                <Form id='add-form' layout="vertical" form={form} 
                        initialValues={{account:"",password:"",phone:"",email:"",sign:""}}>
                    <Row gutter={16}>
                        <Col span={12}>
                            <Form.Item
                                name="account"
